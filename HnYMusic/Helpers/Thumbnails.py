...
async def thumb_init(videoid):
    try:
        if os.path.isfile(f"HnYMusic/Cache/{videoid}.jpg"):
            return f"HnYMusic/Cache/{videoid}.jpg"
        ...
        if 1==1:
                ...
                async with session.get(f"http://img.youtube.com/vi/{videoid}/maxresdefault.jpg") as resp:
                    if resp.status == 200:
                        f = await aiofiles.open(
                            f"HnYMusic/Cache/thumb{videoid}.jpg", mode="wb"
                        )
                        await f.write(await resp.read())
                        await f.close()

            youtube = Image.open(f"HnYMusic/Cache/thumb{videoid}.jpg")
            ...
            # fonts
            font1 = ImageFont.truetype('HnYMusic/Utilities/font.ttf', 30)
            font2 = ImageFont.truetype('HnYMusic/Utilities/font.ttf', 70)
            font3 = ImageFont.truetype('HnYMusic/Utilities/font.ttf', 40)
            font4 = ImageFont.truetype('HnYMusic/Utilities/font.ttf', 35)

            image4 = ImageDraw.Draw(image2)
            image4.text((10, 10), "HnY MUSIC", fill="white", font = font1, align ="left") 
            image4.text((670, 150), "HnY PLAYER", fill="white", font = font2, stroke_width=2, stroke_fill="white", align ="left") 
            ...
            image2.save(f"HnYMusic/Cache/{videoid}.jpg")
            file = f"HnYMusic/Cache/{videoid}.jpg"
